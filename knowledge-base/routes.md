...
The public route subsystem expands the referral-led homepage into substantive editorial pages and a private enquiry workflow without turning the site into a package catalogue. It covers How We Work, Travel Inspiration, About, and Plan Your Journey.
...
## How it is tested
Run `pnpm test` for content, shell, navigation, metadata, form, and per-route render contracts, then `pnpm verify`. Direct-load all four interior paths through `next start` and browser-test at 390 x 844 and 1440 x 900. Confirm cross-route navigation, `/` home, `/plan-your-journey`, form states, focus-visible behavior, no overflow, and no console errors.
